...
//    colorSubChase(&imgData[i], strip.Color(color, 100 - color, 0), startEnd[i], startEnd[i+1]-1); // White
    colorSubChase(strip.Color(color, 100 - color, 0), startEnd[i], startEnd[i+1]); // White
...
void colorSubChase(uint32_t c, word startPixel, word endPixel) {
  int j;
  ...
  for(j=startPixel; j<endPixel; j++) {
    ...
    strip.setPixelColor(j, c); // Set new pixel 'on
    strip.setPixelColor(j-1, 0); // set the previous pixel off
    strip.show();
  }
  
  strip.setPixelColor(endPixel-1, 0);
  strip.show();  // Turn off final pixel
}
//
